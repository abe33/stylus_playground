...
The `widgets.$define` is a shortcut to define a jQuery mixin.

    widgets.$define = (name, block) ->
      __widgets__[name] = (element, options) ->
        $element = $(element)
        $element[name](options) if $.fn[name]?
        block?.call($element, element, options)

### widgets.is_defined

    widgets.is_defined = (name) -> __widgets__[name]?
